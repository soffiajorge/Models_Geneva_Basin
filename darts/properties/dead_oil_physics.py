...
from properties.dead_oil_operator_python import *
    ...
    def __init__(self, timer, n_points, min_p, max_p, min_z, property_container, grav):
        ...
        if grav:
            try:
                ...
                self.acc_flux_itor_well = acc_flux_itor_name(self.acc_flux_w_etor, index_vector([n_points, n_points]),
                                                    value_vector([min_p, min_z]), value_vector([max_p, 1 - min_z]))
            except RuntimeError:
                self.acc_flux_itor = acc_flux_itor_name_long(self.acc_flux_etor, index_vector([n_points, n_points]),
                                                        value_vector([min_p, min_z]), value_vector([max_p, 1 - min_z]))
                self.acc_flux_itor_well = acc_flux_itor_name_long(self.acc_flux_w_etor, index_vector([n_points, n_points]),
        ...
        self.new_rate_water_prod = lambda rate: rate_prod_well_control(self.phases, 0, self.nr_components,
