...
class DensityWater(property_evaluator_iface):
    def __init__(self, pref = 1, Bref = 1.01, compres = 4.45e-5, surface_wat_dens = 1025):
        super().__init__()
...
class ViscosityOil(property_evaluator_iface):
    def __init__(self, pref=1, Viscoref=3, viscosibility=0):
        super().__init__()
        self.Pref = pref
...
class ViscosityWater(property_evaluator_iface):
    def __init__(self, pref = 1, Viscoref = 0.3, viscosibility = 0):
        super().__init__()
        self.Pref = pref
        self.Viscoref = Viscoref
        self.viscosibility = viscosibility
...
class PhaseRelPerm(property_evaluator_iface):
    def __init__(self, phase, swc=0.2, sor=0.2, krwe=1.0, kroe=1.0, nw=2, no=2):
        super().__init__()
        self.phase = phase
        self.swc = swc
        self.sor = sor
...
class CapillaryPressure(property_evaluator_iface):
    def __init__(self, pentry=0.2, lam=0.5, swc=0.2, sor=0.2):
        super().__init__()
        self.pd = pentry
        self.lam = lam
        self.swc = swc
